...
class OrdersProvider extends BaseProvider {
  static const String _Table_Orders = 'Orders';

  String GetTable() {return _Table_Orders;}

  ///插入一条消息
  Future<int> insert(Orders msg) async {
  ...
  ///修改表
  Future<int> update(Orders msg, String field, String parameter) async {
    Database? db = await getDataBase();
  ...
  ///查询表
  ///根据id查询数据
  Future<Orders?> queryTableBySingleField(String field, String parameter) async{
    Database? db = await getDataBase();
    List<Map<String, dynamic>> maps = await db!.query(GetTable(), where: "$field = ?", whereArgs: [parameter]);
    if(maps.isNotEmpty) {
      Orders msg =Orders.fromJson(maps.first);
      print("msg-$msg");
      return msg;
    }
    return null;
  ...
  ///查询全部数据列表
  Future<List<Orders>?> queryTableAll() async{
    Database? db = await getDataBase();
    List<Map<String, dynamic>> maps = await db!.query(GetTable());
    if(maps.isNotEmpty) {
      List<Orders> msgs = maps.map((item)=>Orders.fromJson(item)).toList();
      print("msgs-$msgs");
      return msgs;
    }
    return null;
