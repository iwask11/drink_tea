...
class UserAddressProvider extends BaseProvider {
  static const String _Table_UserAddress = 'UserAddress';

  String GetTable() {return _Table_UserAddress;}

  ///插入一条消息
  Future<int> insert(UserAddress msg) async {
  ...
  ///修改表
  Future<int> update(UserAddress msg, String field, String parameter) async {
    Database? db = await getDataBase();
  ...
  ///查询表
  ///根据id查询数据
  Future<UserAddress?> queryTableBySingleField(String field, String parameter) async{
    Database? db = await getDataBase();
    List<Map<String, dynamic>> maps = await db!.query(GetTable(), where: "$field = ?", whereArgs: [parameter]);
    if(maps.isNotEmpty) {
      UserAddress msg =UserAddress.fromJson(maps.first);
      print("msg-$msg");
      return msg;
    }
    return null;
  ...
  ///查询全部数据列表
  Future<List<UserAddress>?> queryTableAll() async{
    Database? db = await getDataBase();
    List<Map<String, dynamic>> maps = await db!.query(GetTable());
    if(maps.isNotEmpty) {
      List<UserAddress> msgs = maps.map((item)=>UserAddress.fromJson(item)).toList();
      print("msgs-$msgs");
      return msgs;
    }
    return null;
